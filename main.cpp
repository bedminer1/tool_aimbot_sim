...
constexpr double kManualPitchRate = 2.4;  // rad/s command generated by W/S
...
int main(int argc, char** argv)
{
    ...
    while (!glfwWindowShouldClose(w)) {
        ...
        while (d->time - frame_start < kRenderDt) {
            ...
            double yaw_manual = 0.0;
            double pitch_manual = 0.0;
            if (glfwGetKey(w, GLFW_KEY_A) == GLFW_PRESS) yaw_manual += 1.0;
            if (glfwGetKey(w, GLFW_KEY_D) == GLFW_PRESS) yaw_manual -= 1.0;
            if (glfwGetKey(w, GLFW_KEY_W) == GLFW_PRESS) pitch_manual += 1.0;
            if (glfwGetKey(w, GLFW_KEY_S) == GLFW_PRESS) pitch_manual -= 1.0;

            command.control = true;
            command.found = true;
            command.yaw_vel = yaw_manual * kManualYawRate;
            command.pitch_vel = pitch_manual * kManualPitchRate;
            ...
            if (pitch_manual != 0.0) {
                command.pitch = std::clamp(
                  command.pitch + command.pitch_vel * m->opt.timestep, kPitchMin, kPitchMax);
            } else {
                command.pitch = d->qpos[pitch_qpos_addr];
            }

            // Manual prototype: kinematic velocity command so controls feel immediate.
            ...
            d->ctrl[pitch_motor] = std::clamp(command.pitch_vel, -kMaxPitchVel, kMaxPitchVel);
        ...
        // Camera mouse controls are only for free camera mode. The sim starts in gimbal POV.
        double nx, ny;
        ...
        std::snprintf(
          left,
          sizeof(left),
          "A/D:yaw W/S:pitch F:camera POV/free R:reset Esc:quit\n"
